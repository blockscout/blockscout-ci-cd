...
test(`@Ethereum @Transactions @Data @AccountImage Check transactions list (new frontend)`, async ({ transactionsListPage }) => {
    const {
        TestNFTSymbolV,
        TestTokenSymbolV,
        TestNFTSymbol,
...
test(`@Ethereum @Transactions @Data @AccountImage Check contract creation tx props (new frontend)`, async ({ transactionPage }) => {
